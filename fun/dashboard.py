    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        if config_path.exists():
            try:
                ...
                config = {}
        setup = {"configured": bool(config.get("base_url") and config.get("model") and (config.get("api_key") or config.get("api_key_env"))), "config_path": str(config_path)}
...
async function load(){const d=await fetch('/api/summary').then(r=>r.json());document.querySelector('#setup').innerHTML=d.setup.configured?'<strong>✓ Provider configured</strong><p>Fun is ready to run model tasks.</p>':'<strong>First run setup</strong><p>Provider is not configured. Run <code>fun --configure</code>, or export <code>FUN_API_URL</code>, <code>FUN_API_KEY</code>, and <code>FUN_MODEL</code>.</p>';document.querySelector('#cards').innerHTML=Object.keys(labels).map(k=>`<div class="card"><div class="label">${labels[k]}</div><div class="value">${d[k]??0}</div></div>`).join('');document.querySelector('#sessions').innerHTML=d.session_usage.map(x=>`<tr><td><code>${x.session_id}</code></td><td>${x.tasks}</td><td>${x.input_tokens}</td><td>${x.output_tokens}</td><td>${x.total_tokens}</td><td>${x.tool_calls}</td></tr>`).join('')||'<tr><td colspan="6">No sessions yet.</td></tr>';document.querySelector('#background').innerHTML=(d.background_tasks||[]).map(x=>`<tr><td><code>${x.task_id}</code></td><td>${x.kind||'—'}</td><td>${x.status}</td><td>${x.parent_task_id||'—'}</td><td>${x.goal||'—'}</td></tr>`).join('')||'<tr><td colspan="5">No background agents yet.</td></tr>';document.querySelector('#recent').innerHTML=d.recent.map(x=>`<tr><td>${new Date(x.timestamp).toLocaleString()}</td><td><code>${x.type}</code></td><td>${x.task_id||'—'}</td><td>${x.goal||x.reason||'—'}</td></tr>`).join('')||'<tr><td colspan="4">No activity yet.</td></tr>'}load();setInterval(load,5000);
</script></body></html>"""
