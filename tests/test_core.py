...
class CoreTests(unittest.TestCase):
    ...
    def test_terminal_ui_keeps_transcript_and_tool_cards_together(self):
        ...
        approval = TerminalUiState(mode="approval")
        self.assertIn("waiting for approval", approval.render())
        ...
        empty.add_user("two")
        empty.scroll(1)
        self.assertIn("older messages", empty.render())
        narrow = TerminalUiState()
        narrow.add_user("a very long user message that should wrap instead of overflowing the terminal")
        self.assertTrue(all(len(line) <= 40 for line in narrow.render(40).splitlines()))
        ...
        self.assertIn("command='echo hi'", recovery_text)
        restored = TerminalUiState()
        restored.restore_messages([{"role": "user", "content": "old goal"}, {"role": "assistant", "content": "old answer"}, {"role": "tool", "tool_call_id": "t1", "content": "old output"}])
        restored_text = restored.render()
        self.assertIn("old goal", restored_text)
        self.assertIn("old answer", restored_text)
        self.assertIn("old output", restored_text)
